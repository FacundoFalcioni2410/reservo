...
export default function ProNavShell({ tenant, children }: { tenant: Tenant; children: React.ReactNode }) {
  const [open, setOpen] = useState(false)

  return (
    <div className="flex h-screen bg-zinc-50">
      <div className="hidden lg:flex">
        <ProSidebar tenant={tenant} />
      </div>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <div className="relative z-50">
            <ProSidebar tenant={tenant} onClose={() => setOpen(false)} />
          </div>
        </div>
      )}
